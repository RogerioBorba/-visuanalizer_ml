...
<script>
	...
	import {addWMSLayer} from '$lib/component/maplibre/maplibreWMS';
	/**
     * @type {WMSLayer} wmsLayer
     */
	export let wmsLayer;
    /**
     * @type {string}
	...
	async function btnAddLayerClicked() {
		...
		await addWMSLayer($map, wmsLayer.sourceLayer, wmsLayer.name())
...
<div class="flex {display}">
	<div role="button" tabindex="0" on:click={toggleExpansion} 
...
{#if children.length > 0  && expanded}
    <ul><!-- transition:slide -->
        {#each children as child}
            <li class="">
				<svelte:self wmsLayer={childLayer(child)} capabilitiesUrl= {capabilitiesUrl} />
